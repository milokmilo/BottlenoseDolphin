...
#'as a starting value for omega in the Galucci and Quinn and Mooij et al.
#'methods.
#'
...
#'@seealso \code{\link{growthModels}}, \code{\link{growthModelSim}},
#'\code{\link{vbFuns}}
...
#'
#'Galucci, V.F. and T.J. Quinn II. 1979.  Reparameterizing, fitting, and
...
#'vbStarts(tl~age,data=SpotVA1,type="Francis",tFrancis=c(0,2.5,5))
#'
#'## Simple Example with a Plot
#'vbStarts(tl~age,data=SpotVA1,type="GalucciQuinn",plot=TRUE)
#'
...
vbStarts.default <- function(age,len,type=c("typical","original","BevertonHolt","Francis","GalucciQuinn","Mooij","Schnute","Somers","Somers2","vonBertalanffy"),tFrancis=NULL,methEV=c("means","poly"),meth0=c("poly","yngAge"),plot=FALSE,...) {
  ...
  switch(type,
    ...
    GalucciQuinn={ sv <- list(omega=sLinf*sK,K=sK,t0=st0) },
    ...
    Schnute=,Francis={
      ...
      ifelse(type=="Schnute",ages2use <- range(ages),ages2use <- tFrancis)
  ...
  if (plot) {
    plot(len~jitter(age),pch=19,xlab="Age (jittered)",ylab="Length",main=paste("von B (",type," paramaterization) fit at intial values",sep=""))
    mdl <- vbFuns(type)
    if (type %nin% c("Schnute","Francis")) curve(mdl(x,sv[[1]],sv[[2]],sv[[3]]),from=min(age),to=max(age),col="red",lwd=3,add=TRUE)
    ...
    legend("topleft",legend=paste(names(sv),formatC(unlist(sv),format="f",digits=2),sep=" = "))
  }
