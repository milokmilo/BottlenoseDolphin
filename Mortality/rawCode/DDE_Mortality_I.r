...
lifeTab <- data.frame(year=ageDistr[,"t_age"], ageCl=ageDistr[,"t_age"]+0.5, M=ageDistr[,"n"]) # mortality table
...
gM <- ggplot(lifeTab,aes(year,Z)) + geom_point()  +
  ylab("Mortality-at-age") + xlab("Years") + 
  ggtitle("DDE F-Mortality")
