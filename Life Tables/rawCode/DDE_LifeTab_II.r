...
StrMod <- list.files("../../RObjects/StrMod")
for (i in 1:length(StrMod)){
  load(paste("../../RObjects/StrMod/", StrMod[i], sep=""))
  modName <- substr(StrMod[i], 1, nchar(StrMod[i])-6)
...
#for (i in 1:length(lifeTabs)){
#  gM <- ggplot(get(lifeTabs[i]),aes(ageCl,Z)) + 
#    geom_line(col=2) + 
#    ylim(0, 1) +
...
unique(GGlifeTab$mod)
gM1 <- ggplot(GGlifeTab[!GGlifeTab$mod=="LifeTabn",],aes(ageCl,qx, group=mod)) + 
  geom_line(col=2) + ylim(0, 1) +
#  facet_grid(mod ~ .) + 
#  geom_line(aes(ageCl,lx), col=4) +
...
#gM2 <- ggplot(GGlifeTab,aes(ageCl,qx, group=mod)) + geom_line(col=2) + ylim(0, 1) +
#  facet_grid(mod ~ .) + geom_line(aes(ageCl,lx), col=4) +
##  geom_line(aes(ageCl,Z*0.4), lty=2, col=2) + geom_line(aes(ageCl,Z*0.6), lty=3, col=2) +
#  ylab("Mortality-at-age / Survivorship") + xlab("Years") + 
#  ggtitle("DDE Survivorship (blue line) and Mortality: Z (red line), F (dotted line), M (dashed line)")
#```{r smallplot, fig.width=16, fig.height=8}
...
##### Plot Report Survivorship/Mortality #####

#png("../plots/DDE_GLM-SM.png", width=600, height=400)
plot(LifeTabn$ageCl, (LifeTabn$nx)/1000, type="lty", col="red", lty=2, 
     ylab="survivorship and mortality", xlab="age", main="Survivorship and Mortality curves for Common dolphin")
lines(LifeTabn$ageCl, LifeTabn$qx, col="blue", lty=2)
lines(LifeTabStr02$ageCl, (LifeTabStr02$nx)/1000, col="red", lty=1)
lines(LifeTabStr02$ageCl, LifeTabStr02$qx, col="blue", lty=1)
legend(10,1,legend=c("ActualData-Surv","ActualData-Mort","GLM-Surv","GLM-Mort"), lty=c(2,2,1,1), col=c("red", "blue","red", "blue"))
#dev.off()
