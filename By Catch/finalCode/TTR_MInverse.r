####################################################################################################
#                    MAXIMUM MORTALITY TO KEEP BOTTLENOSE POPULATION STABLE
#       Maximum level of total mortality than the population can accept to keep constant
#                   created: (camilo.saavedra@vi.ieo.es) 24/04/2015
####################################################################################################
...
load(file="../../RObjects/lifeSilerA-S&B.RData")
NatMort <- lifeSilerA$qx
...
load("../../RObjects/TTR_Mat-Wells.RData")
TTRMatFema <- ggMat[ggMat$sex %in% "F",]
TTRMatFema <- TTRMatFema[TTRMatFema$age %in% seq(0,49),]
# Moving one age up and delete the last age
#Mat <- Mat[-length(Mat)]
#Mat <- c(0,Mat)
...
fun <- function(x){
  laZ <- M + F * x
  life <- data.frame(age=0:35,qx=laZ)
  ...
  nx <- nx[-length(nx)]
  pop <- nx/sum(nx) * p 
  birth <- sum((pop/2)*TTRMatFema$matPred[1:35])/pr
...
M <- NatMort 
  #F <- Z_ByC[,i] * (by * 1.25) Necesitamos saber cuál es el patrón de bycatch para el delfín mular
F <- rep(0.1,36)# Mientras le damos unos valores constantes de bycatch
#  F <- Z_ByC[,2] - M_ByC[,2] 
for (j in 1:length(preg)){
    pr <- preg[j]
    assign(paste("MaxMort","_", preg[j], sep=""), 
           optimize(f=fun, interval=c(-1,5)))
}
 # We obtain the times that bycatch should be reduced

l <- ls(pattern="MaxMort_[+0-9]")
...
for (i in 2:length(df)){
    ...
    for (k in 1:length(preg)){
      if (k == 1){ 
        birth <- sum((pop[-length(pop)]/2)*TTRMatFema)/preg[k]} 
      else{
        birth <- c(birth, sum((pop[-length(pop)]/2)*TTRMatFema)/preg[k])  
      }
      if ( k == length(preg)){
