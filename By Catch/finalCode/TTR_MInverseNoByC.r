...
TTRMatFem <- ggMat[ggMat$sex %in% "F",]
TTRMatFem <- TTRMatFem[TTRMatFem$age %in% seq(0,34,1),]
...
fun <- function(x){
  ...
  nx <- nx[-length(nx)]
  pop <- nx/sum(nx) * p 
  birth <- sum((pop/2)*TTRMatFem$matPred)/pr
...
M <- lifeSilerA$qx
#F <- Z_ByC[,i] * (by * 1.25) # Necesitamos los datos de bycatch para calcular las proporciones por edad
F <- rep(0.1,36)
#  F <- Z_ByC[,2] - M_ByC[,2] 
for (j in 1:length(preg)){
    pr <- preg[j]
    assign(paste("MaxMat","_", preg[j], sep=""), 
           optimize(f=fun, interval=c(-1,5)))
}
# We obtain the times that bycatch should be reduced

l <- ls(pattern="MaxMat_[+0-9]")
...
dfpop <- data.frame(age=seq(0,nrow(df)-1))
dfpreg <- data.frame(preg=round(100/preg))
for (i in 3:length(df)){
    life <- data.frame(age=0:35,qx=df[,i])
    for (j in 1:nrow(life)){
      if (j == 1) {dx <- life$qx[1]*n; nx <- n
      } else {
        nx <- c(nx, nx[j-1]- dx[j-1])
    ...
    for (k in 1:length(preg)){
      if (k == 1){ 
        birth <- sum((pop[-length(pop)]/2)*TTRMatFem$matPred)/preg[k]} 
      else{
        birth <- c(birth, sum((pop[-length(pop)]/2)*TTRMatFem$matPred)/preg[k])  
      }
      if ( k == length(preg)){
